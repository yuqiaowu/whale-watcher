...
-   **双重时间窗口 (Dual Timeframe Context):** AI 同时接收 **24小时 (短期异动)** 和 **7天 (中期趋势)** 的数据，通过对比分析来识别“短期抛压 vs 长期吸筹”的背离信号。
-   **结构化输出:** 前端引入 Markdown 渲染引擎，支持 AI 生成结构化的要点（Bullet Points）和高亮关键词（Bold），极大提升了阅读体验。
...
-   **我的实践:** 该系统是 **全链适用（Chain-Agnostic）** 的。
    ...
    -   **Solana (SVM):** 处理 Solana DEX 独特的“Swap”逻辑。
        *   *工程取舍 (Trade-off):* 针对 Solana 高达数千的 TPS，采用了 **“深度翻页 (Deep Pagination)”** 策略（单次抓取 ~1000 笔交易快照）而非全量流式传输。这是在 MVP 阶段平衡 **API 成本** 与 **数据覆盖度** 的最佳实践。
...
**Q: Solana TPS 那么高，你怎么保证监控的数据有效性？**
*A: “这是一个经典的工程取舍（Build vs Buy）。在没有预算接入昂贵的企业级全量索引服务（如 The Graph 专用节点）时，我采用了‘快照抽样’策略：每次分析时深度抓取过去 1000 笔交易。虽然这牺牲了毫秒级的实时性，但对于捕捉‘巨鲸趋势’来说，样本量已经足够具有统计学意义。”*

**Q: 为什么要做 24h 和 7天 双重周期分析？**
*A: “为了解决‘数据说谎’的问题。有时候单纯看 24h 数据显示‘大额流出（利好）’，但拉长到 7 天看其实是‘持续性抛压后的离场（利空）’。我设计的双重周期让 AI 能够像资深交易员一样，通过长短周期的背离来发现更深层的市场信号。”*
