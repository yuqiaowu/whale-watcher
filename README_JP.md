...
| 日付 | 主な更新内容 |
| :--- | :--- |
| **2026-02-26** | **V2.0 メジャーアップグレード**: Qlib モデルスコアリング、Z-Score 異常検知、強化されたリスク管理システムを統合。フロントエンドに定量分析と安全状況モジュールを追加、Live Qlib Bridge でのリアルタイムデータ対応、多言語 (UIおよびドキュメント) の完全対応を完了。 |
| **2026-02-25** | **多言語ローカリゼーション**: フロントエンド (ニュース、AI取引分析) が完全なバイリンガル切り替えに対応。寄付チャネルを更新し、Solana Blink ワンクリック支払いをサポート。 |
| **2026-02-24** | **システムリスク管理制約**: 極端なパラメータによる実盤 API エラーを防ぐため、AI 脳に動的なリスク管理制約のプロンプトを注入。データベースのポートフォリオ NAV 履歴の自己修復能力を強化。 |
| **2026-02-23** | **AI エージェント能力の進化**: 大規模言語モデルの記憶と実盤取引の履歴を分離し、ポジションの錯覚や混乱を防止。入力ソースに7日周期のクジラ資金流向トレンドを統合し、NAV 計算のベースラインを安定化。 |
